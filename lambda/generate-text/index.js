...
exports.handler = async (event) => {
  try {
    ...
    const prompt = `Generate text for shadow reading practice on the topic of "${topic.replace('_', ' ')}". The text should be suitable for a ${difficulty} level, using ${difficultyPrompts[difficulty]}. Ensure the text is concise, clear, and appropriate for reading aloud. Avoid lists, dialogue, or overly technical terms unless relevant to the topic. Do not include any introductory phrases or titles - return only the text content itself. End with a period or question mark.`;
    ...
    // Clean up the response if needed (as an extra precaution)
    generatedText = generatedText.replace(/^Shadow reading practice on the topic of .+:\s*/i, '');
